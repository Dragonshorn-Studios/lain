...
pnpm lainctl reconcile SERVICE_ID`}</Code>
        ...
        <DocSection id="security" eyebrow="09 / Trust" title="Security model">
          <div className="grid gap-x-10 gap-y-6 md:grid-cols-2"><Concept icon={<Server/>} title="Single admin">The dashboard uses one admin password with server-side sessions; machines use API keys. Same-origin is enforced by default, and laind refuses to run live adapters with authentication disabled.</Concept><Concept icon={<LockKeyhole/>} title="Least privilege">Give the Cloudflare token only DNS edit and Tunnel edit permissions for the selected resources.</Concept></div>
          <ul className="list-disc space-y-2 pl-5"><li>Complete first-run setup immediately after installation — whoever sets the admin password first owns the dashboard.</li><li>Machines authenticate with API keys: set <InlineCode>LAIN_API_KEY</InlineCode> on headless servers, or store a key in the OS keychain with <InlineCode>lainctl auth login</InlineCode>. Roll keys from the API keys page.</li><li>Do not expose the API directly to the public internet; the dashboard belongs on your trusted LAN.</li><li>Keep credential source files, <InlineCode>.env</InlineCode>, the database, and certificate directory private.</li><li>Remove temporary source token files after host setup succeeds.</li><li>Review setup plans before confirmation; reserve <InlineCode>--yes</InlineCode> for audited automation.</li></ul>
        </DocSection>
      </article>
    </div>
  </>;
