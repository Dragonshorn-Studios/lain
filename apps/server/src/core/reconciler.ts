...
export class Reconciler {
  ...
  constructor(private readonly repository: ServiceRepository, private readonly adapters: DerivedStateAdapter[]) {}
  async reconcileService(service: Service): Promise<ComponentStatus[]> {
    ...
    for (const adapter of this.adapters) {
      try {
        ...
        const result = { component: adapter.component, desired: "configured", actual: "error", state: "error" as const, message: error instanceof Error ? error.message : "Unknown reconciliation error" };
        this.repository.setComponentStatus(service.id, result);
        statuses.push({ ...result, reconciledAt: new Date().toISOString() });
      }
    }
