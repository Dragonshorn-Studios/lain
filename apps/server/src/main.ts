...
const reconciler = new Reconciler(repository, [new LocalStateAdapter("dns"), new LocalStateAdapter("proxy"), ...externalAdapters]);
...
const api = await buildApi(config, repository, reconciler, health, dns);
